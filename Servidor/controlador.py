...
##
##ESte es un comment para generar un PR
...
class Controlador:
    ...
    def conectar_robot(self):
        try:
            ...
            print(
                f"Conexión establecida en {self.puerto_COM} con baudrate {self.baudrate}."
            )
        except serial.SerialException as e:
            print(
                f"Error al conectar: Verifique que el puerto {self.puerto_COM} esté disponible y correcto."
            )
    ...
    def enviar_comando(self, comando):
        if self.estado_robot:
            try:
                ...
                respuesta = (
                    self.arduino.readline().decode("utf-8").strip()
                )  # Leer respuesta
                if respuesta:
